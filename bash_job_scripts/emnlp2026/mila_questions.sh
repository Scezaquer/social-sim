...
# Deterministic mapping: graphs(7) x homophily(2) x questions(3) x runs(5) = 210
if [[ -z "${SLURM_ARRAY_TASK_ID:-}" ]]; then
    echo "SLURM_ARRAY_TASK_ID is not set. Run as job array (1-210)." >&2
    exit 1
fi

idx=$((SLURM_ARRAY_TASK_ID - 1))
runs_per=5
num_questions=${#QUESTION_CHOICES[@]}
...
question_index=$(( idx % num_questions ))
idx=$(( idx / num_questions ))
...
QUESTION_PICK="${QUESTION_CHOICES[$question_index]}"
QUESTION_NUMBER="${QUESTION_PICK%%:*}"
TWEET_FILE="${QUESTION_PICK#*:}"
